...
{
    public static DataExporterManager Ins { get; } = new();

    private readonly Dictionary<string, IDataExporter> _dataExporters = new();
    private readonly Dictionary<string, ITableExporter> _tableExporters = new();
    ...
    public ITableExporter GetTableExporter(string name)
    {
    ...
    public void RegisterTableExporter(string name, ITableExporter exporter)
    {
    ...
    public void ScanRegisterTableExporter(Assembly assembly)
    {
        foreach (var t in assembly.GetTypes())
        {
            ...
            if (t.GetCustomAttribute<TableExporterAttribute>() is { } attr)
            {
                var exporter = Activator.CreateInstance(t) as ITableExporter;
